    ...
    <div class="bg-white p-6 rounded-2xl shadow-2xl w-[36rem] max-h-[85%] flex flex-col">
      <div class="flex justify-between items-center mb-6 border-b pb-3">
        <div class="flex items-center space-x-4">
          ...
          <button v-if="trashList.length > 0" @click="$emit('requestEmpty')" class="text-sm bg-red-100 text-red-600 px-3 py-1.5 rounded-lg hover:bg-red-200 font-bold transition shadow-sm">一键清空</button>
        </div>
      ...
      <div class="flex-1 overflow-y-auto space-y-3 pr-2">
        ...
        <div v-for="item in trashList" :key="item.id" class="flex justify-between items-center bg-gray-50 p-4 rounded-xl border border-gray-100 shadow-sm">
          ...
          <div class="flex space-x-2 shrink-0">
            ...
            <button @click="$emit('requestPermDelete', item)" class="bg-red-100 text-red-600 px-3 py-1.5 rounded-lg font-bold hover:bg-red-200 transition text-sm">彻底删除</button>
          </div>
        </div>
      </div>
    </div>
...
defineEmits(['close', 'restore', 'requestPermDelete', 'requestEmpty']);
</script>