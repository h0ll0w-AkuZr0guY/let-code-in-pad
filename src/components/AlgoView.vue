  ...
  <div class="flex-1 flex overflow-hidden relative transition-colors" :class="{'select-none': isDragging}">
    <div class="flex flex-col bg-white dark:bg-gray-900 transition-colors" :style="{ width: leftWidth + '%' }">
      ...
      <div class="p-6 flex-1 overflow-y-auto">
        <textarea v-if="isEditing" v-model="form.problemText" @paste="(e) => handleSmartPaste(e, form, 'problemText')" class="w-full h-full resize-none outline-none text-lg p-2 border-2 border-dashed border-gray-200 dark:border-gray-700 bg-transparent dark:text-gray-200 rounded-lg focus:border-blue-400 dark:focus:border-blue-500 transition"></textarea>
        <div v-else class="prose dark:prose-invert max-w-none text-lg" v-html="renderMarkdown(form.problemText, form.images)"></div>
      </div>
    </div>
    ...
    <div class="flex flex-col bg-[#1e1e1e] dark:bg-black transition-colors" :style="{ width: (100 - leftWidth) + '%' }">
      ...
      <div class="p-6 flex-1 overflow-y-auto">
        <textarea v-if="isEditing" v-model="form.solutionText" @paste="(e) => handleSmartPaste(e, form, 'solutionText')" class="w-full h-full resize-none outline-none text-lg p-2 bg-transparent text-gray-200 font-mono border-2 border-dashed border-gray-600 rounded-lg focus:border-blue-400"></textarea>
