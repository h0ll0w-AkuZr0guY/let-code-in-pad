  ...
  <transition enter-active-class="transition-transform duration-300 ease-out" enter-from-class="translate-x-full" enter-to-class="translate-x-0" leave-active-class="transition-transform duration-200 ease-in" leave-from-class="translate-x-0" leave-to-class="translate-x-full">
    <div v-if="show" class="absolute top-0 right-0 w-72 h-full bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-l border-gray-200 dark:border-gray-800 shadow-2xl flex flex-col z-30">
      ...
      <div class="p-5 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center shrink-0 bg-gray-50/50 dark:bg-gray-800/30">
        <h3 class="font-bold text-gray-800 dark:text-gray-200 tracking-wider text-sm flex items-center"><span class="text-blue-500 mr-2 text-lg">🗂️</span> 内容导航</h3>
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-0.5 pb-24 scroll-smooth">
        <div v-if="headings.length === 0" class="text-sm text-gray-400 dark:text-gray-500 text-center mt-12 flex flex-col items-center"><span class="text-3xl mb-2 opacity-50">📄</span>正文暂无标题结构</div>
        
        <div v-for="h in headings" :key="h.slug" @click="scrollTo(h.slug)"
             ...
             :class="[ h.level === 1 ? 'mt-2 mb-1' : '', h.level === 2 ? 'ml-3' : '', h.level === 3 ? 'ml-7' : '', h.level === 4 ? 'ml-10' : '' ]" :title="h.rawTitle">
          ...
          <span class="truncate transition-colors"
                :class="[ h.level === 1 ? 'font-bold text-gray-800 dark:text-gray-100 group-hover:text-blue-600 dark:group-hover:text-blue-400' : h.level === 2 ? 'font-medium text-gray-600 dark:text-gray-300 group-hover:text-blue-600 dark:group-hover:text-blue-400' : 'text-gray-500 dark:text-gray-400 group-hover:text-blue-500 dark:group-hover:text-blue-300' ]"
                :style="{ fontSize: getTocFontSize(h.level) }">
            {{ h.title }}
          </span>
        </div>
      </div>
...
const props = defineProps(['show', 'form', 'contentFontSize']);
defineEmits(['close']);

const getTocFontSize = (level) => {
  // 核心逻辑：基础字号被钳制在 12px ~ 17px 之间，防止过大或过小
  let base = Math.max(12, Math.min(17, (props.contentFontSize || 16) * 0.8));
  if (level === 1) return (base + 2) + 'px';
  if (level === 2) return base + 'px';
  return (base - 1) + 'px';
};
...
const headings = computed(() => {
  ...
  return matches.map(m => {
    const level = m[1].length; const rawTitle = m[2].trim();
    const slug = 'toc-' + encodeURIComponent(rawTitle.replace(/[*`_]/g, '').trim().toLowerCase().replace(/\s+/g, '-').substring(0, 30)).replace(/%/g, '');
    let displayTitle = rawTitle.replace(/[*`_]/g, ''); if (displayTitle.length > 20) displayTitle = displayTitle.substring(0, 19) + '...';
